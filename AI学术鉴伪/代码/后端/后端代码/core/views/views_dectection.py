...
class SubDetectionResultSerializer(serializers.ModelSerializer):
    ...
    def get_mask_image(self, obj):
        if obj.mask_image:
            return f"/api/preview/sub_result/{obj.id}/"
        return None
...
def detection_result_detail(request, result_id):
    ...
    add("llm_image",    f"/api/preview/detection/{dr.id}/?image_type=llm")
    add("ela_image",    f"/api/preview/detection/{dr.id}/?image_type=ela")
    add("exif", {
