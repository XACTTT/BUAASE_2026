...
def urn_initial_model(path: str, gpu: int = 0):
    current_dir = os.path.dirname(os.path.abspath(__file__))
    model_path = path if os.path.isabs(path) else os.path.join(current_dir, path)
    args = EasyDict({
        "train_set": None,
        "test_set": None,
        "gpu": gpu,
...
if __name__ == '__main__':
    checkpoint_root = os.path.abspath(os.path.join(os.path.dirname(__file__), os.pardir, os.pardir, "checkpoints"))
    args = EasyDict({
        "train_set": None,
        "test_set": None,
        "gpu": 0,
    ...
    hp = get_hp(configs.coarse_bio_hyper_para.hp, args)
    net = get_model(hp)
    checkpoint = torch.load(os.path.join(checkpoint_root, "image_forgery", "Coarse_v2.pkl"), map_location="cpu",
                            weights_only=False)
    net.load_state_dict(checkpoint['model_state_dict'])
    net = net
