...
from service_config import SERVICE_CONFIG, resolve_config_path

current_dir = os.path.dirname(os.path.abspath(__file__))
service_root = os.path.abspath(os.path.join(current_dir, os.pardir, os.pardir))
checkpoint_root = os.path.join(service_root, "checkpoints")
# 构造项目c的绝对路径
...
class SingleImageMethod:
    def __init__(self):
        ...
        self.coarse_v2_net, self.coarse_v2_hp = urn_initial_model(
            str(resolve_config_path(SERVICE_CONFIG["image_forgery_models"]["urn_coarse_v2"])),
            gpu=int(SERVICE_CONFIG["model_devices"]["urn_coarse_v2"]),
        )
        self.blur_net, self.blur_hp = urn_initial_model(
            str(resolve_config_path(SERVICE_CONFIG["image_forgery_models"]["urn_blurring"])),
            gpu=int(SERVICE_CONFIG["model_devices"]["urn_blurring"]),
        )
        self.brute_net, self.brute_hp = urn_initial_model(
            str(resolve_config_path(SERVICE_CONFIG["image_forgery_models"]["urn_brute_force"])),
            gpu=int(SERVICE_CONFIG["model_devices"]["urn_brute_force"]),
        )
        self.contrast_net, self.contrast_hp = urn_initial_model(
            str(resolve_config_path(SERVICE_CONFIG["image_forgery_models"]["urn_contrast"])),
            gpu=int(SERVICE_CONFIG["model_devices"]["urn_contrast"]),
        )
        self.inpating_net, self.inpating_hp = urn_initial_model(
            str(resolve_config_path(SERVICE_CONFIG["image_forgery_models"]["urn_inpainting"])),
            gpu=int(SERVICE_CONFIG["model_devices"]["urn_inpainting"]),
        )
    ...
    def llm_method_single(self, image_path):
        ...
        # 定义路径变量
        WEIGHT_PATH = os.path.join(checkpoint_root, 'llm', 'fakeshield-v1-22b')
        ...
        # 虚拟环境路径
        venv_python = SERVICE_CONFIG["llm_image"].get("venv_python", '/root/miniconda3/envs/llm/bin/python')
        venv_pip = SERVICE_CONFIG["llm_image"].get("venv_pip", '/root/miniconda3/envs/llm/bin/pip')

        # 设置环境变量
        env = os.environ.copy()
        env['CUDA_VISIBLE_DEVICES'] = str(SERVICE_CONFIG["model_devices"].get("llm_image", 0))
