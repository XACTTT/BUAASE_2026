...
def _read_images_from_zip(zip_path: Path) -> List[np.ndarray]:
    ...
    with zipfile.ZipFile(zip_path) as zip_file:
        for zip_info in zip_file.infolist():
            zip_member = Path(zip_info.filename)
            if zip_info.is_dir() or not zip_member.name:
                continue
            if zip_member.suffix.lower() not in {".png", ".jpg", ".jpeg", ".bmp", ".gif"}:
                continue
...
def run_image_pipeline(request_data: Dict[str, Any], *, request_path: Path) -> Dict[str, Any]:
    ...
    if config is None:
        ...
        if config_path and config_path.is_file():
            ...
            config = dict(SERVICE_CONFIG["image_pipeline"]["defaults"])
