...
onMounted(async () => {
  ...
  try {
    ...
    const textListResp = await resourceApi.getTaskTextResults(props.taskId)
    ...
    const detailPromises = results.map(async (item: TextResultItem) => {
      try {
        const detailResp = await resourceApi.getSingleTextResult(item.resource_id)
        ...
        <v-col cols="12" md="4">
          <v-card class="mb-6" elevation="2" rounded="lg">
            <v-card-title class="d-flex align-center pa-6">
              ...
              <v-spacer />
              <v-chip size="small" color="primary" variant="tonal">
                {{ structuredReviewSections.length }} 段
              </v-chip>
            </v-card-title>
            <v-card-text class="pa-4 pt-0">
