...
const downloadReport = async () => {
  try {
    const response = await resourceApi.downloadReport(props.task_id)
...
onMounted(async () => {
  try {
    const taskAnalysisResponse = await resourceApi.getTaskLlmAnalysis(props.task_id)
    taskLlmAnalysis.value = taskAnalysisResponse.data?.llm_analysis ?? null

    detectionResult.value.fakeImages = (await resourceApi.getFakeImages(props.task_id)).data.results
    detectionResult.value.realImages = (await resourceApi.getNormalImages(props.task_id)).data.results
...
const fetchImageDetection = async (result_id: string) => {
  try {
    const response = (await resourceApi.getSingleImageResult(result_id)).data
